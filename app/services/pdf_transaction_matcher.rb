# frozen_string_literal: true

class PdfTransactionMatcher
  MATCH_WEIGHT_AMOUNT = 0.6
  MATCH_WEIGHT_DATE   = 0.2
  MATCH_WEIGHT_DESC   = 0.1

  DATE_TOLERANCE_DAYS = 1
  AMOUNT_TOLERANCE    = 1.0

  attr_reader :transactions, :user, :start_date, :end_date, :exclude_entries

  def initialize(transactions, user, start_date: nil, end_date: nil, exclude_entries: [])
    @transactions    = transactions
    @user            = user
    @start_date      = start_date
    @end_date        = end_date
    @exclude_entries = exclude_entries.to_a
  end

  # Normalise les descriptions (minuscules, sans caractères spéciaux)
  def normalize(str)
    str.to_s.downcase.gsub(/[^a-z0-9\s]/, "").strip
  end

  def match_all
    transactions.map do |tx|
      best_match, best_score = find_best_match(tx)

      {
        id:             tx[:id],
        date:           tx[:date],
        description:    tx[:description],
        amount:         tx[:amount],
        type:           tx[:type], # ignoré pour le matching mais gardé si tu veux l'afficher
        matchable:      best_match,
        matchable_type: best_match&.class&.name,
        matchable_id:   best_match&.id,
        match_score:    best_score
      }
    end
  end

  private

  def find_best_match(tx)
    best_score = 0.0
    best_entry = nil

    # 🔥 On récupère séparément dépenses et revenus,
    # puis on les fusionne en ARRAY Ruby (plus de .or SQL)
    expenses = user.expenses
    revenues = user.revenues

    if start_date && end_date
      expenses = expenses.where(date: start_date..end_date)
      revenues = revenues.where(date: start_date..end_date)
    end

    entries = expenses.to_a + revenues.to_a

    # Exclure les entrées déjà validées
    entries.reject! do |e|
      exclude_entries.include?([e.class.name, e.id])
    end

    entries.each do |entry|
      # 1) Score date
      date_score =
        if tx[:date] && entry.date && (tx[:date] - entry.date).abs <= DATE_TOLERANCE_DAYS
          1.0
        else
          0.0
        end

      # 2) Score montant (en ignorant les signes)
      tx_amount    = tx[:amount].to_f.abs
      entry_amount = entry.amount.to_f.abs
      amount_score = (tx_amount - entry_amount).abs <= AMOUNT_TOLERANCE ? 1.0 : 0.0

      # 3) Score description
      tx_desc    = normalize(tx[:description])
      entry_desc = normalize(entry.description)

      lev_score =
        if tx_desc.empty? || entry_desc.empty?
          0.0
        else
          distance = Text::Levenshtein.distance(tx_desc, entry_desc).to_f
          max_len  = [tx_desc.length, entry_desc.length].max
          1.0 - (distance / max_len)
        end

      contains_score =
        if tx_desc.empty? || entry_desc.empty?
          0.0
        else
          (tx_desc.include?(entry_desc) || entry_desc.include?(tx_desc)) ? 1.0 : 0.0
        end

      desc_score = [lev_score, contains_score].max

      # 4) Score global pondéré
      total_score =
        (amount_score * MATCH_WEIGHT_AMOUNT) +
        (date_score   * MATCH_WEIGHT_DATE)   +
        (desc_score   * MATCH_WEIGHT_DESC)

      # Montant + date exacts => score minimum 90 %
      if date_score == 1.0 && amount_score == 1.0
        total_score = [total_score, 0.9].max
      end

      if total_score > best_score
        best_score = total_score
        best_entry = entry
      end
    end

    [best_entry, best_score]
  end
end
